...
def render_result_details(
    ...
    with left:
        st.plotly_chart(
            scheduling_viz.plot_schedule_map(
                nests,
                tasks,
                result,
                zones=current_demo_zones(prefix),
                title=f"{result['algorithm']} 路径结果",
            ),
            use_container_width=True,
        )
    with right:
        st.plotly_chart(scheduling_viz.plot_uav_task_counts(result), use_container_width=True)
...
def render_dynamic_insertion_platform() -> None:
    ...
    if b2.button("新增突发任务并尝试插入", type="primary", use_container_width=True, key="dynamic_insert"):
        ...
        emergency_df = scheduling_data.generate_emergency_task(
            tasks,
            map_size=float(map_size),
            random_seed=seed,
            release_time=float(release_time),
        )
        insertion = scheduling_dynamic.insert_emergency_task(
    ...
    st.plotly_chart(scheduling_viz.plot_dynamic_metric_compare(before_metrics, after_metrics), use_container_width=True)
    st.markdown("#### 插入后路径摘要")
